...
- **Paint ("Paint")** — only unlocked when the user has uploaded a Photo. Replaces the visualizer pipeline entirely with `PaintingMode`, which animates the photo in place via canvas2D `drawImage` ops. Audio signal (`audio.bass`/`mid`/`treble`) modulates displacement magnitudes, phase timing, and bloom strength. `_loop` short-circuits after signal updates — director, visualizers, and `_renderMain` are skipped. Journey camera is not created in this mode.
  - **Phase director**: `PaintingMode.phases` is a 7-entry array — `bloom`, `mandala`, `liquid`, `vortex`, `shatter`, `chroma`, `slitscan`. `update(dt,audio)` advances `phaseT` and calls `_cut()` when `phaseT > phaseDur` (14–22 s, shorter on bass cuts) or on a bass spike `>0.82` rate-limited by `bassCoolT=4`. Each cut picks a different phase, resets `phaseFade` to 0, and has a 35 % chance to flip `this.mirror` (which toggles `src` between `photoCv` and `photoMirrorCv`). During the 1.6 s crossfade both `prevPhase` and the new phase render with complementary alphas via `_renderPhase(name, src, audio, a)`.
  - **Continuous base**: every frame, independent of phase, a dim `rgba(4,0,18,…)` fill is applied over `buf`, then `buf` is redrawn into itself with a tiny rotate (`sin(t*0.31)*0.0007 + sin(t*1.2)*audio.mid*0.004`) and breathing scale (`1.002 + 0.006*sin(t*0.42) + audio.bass*0.01`). This self-feedback is what produces the slow psychedelic drift behind every phase. A screen-blended bass bloom re-draws `src` scaled up on `audio.bass > 0.55`.
  - **Phase methods**: `_phaseBloom` 36 horizontal melt strips + alternating 24-column shimmer; `_phaseMandala` 6/8/10-fold radial kaleidoscope with alternating mirror wedges; `_phaseLiquid` 22×14 tile grid displaced by composed-sine flow field; `_phaseVortex` 8 concentric ring clips rotated by `baseRot*(1+i*0.15)` for a twirl; `_phaseShatter` 9×9 tile grid with per-tile independent drift and rotation; `_phaseChroma` chromatic split via `ctx.filter='hue-rotate(±55deg)'` plus a 140° echo; `_phaseSlitscan` fine row shifts with occasional hue-rotated glitch bands.
  - **Ripples**: bass `>0.62` (with cooldown) spawns an entry in `this.ripples` — an expanding annulus clip (`arc` + reversed `arc`) of the *unmirrored* `photoCv` with a per-ripple `hue-rotate` filter, `_drawRipples` called every frame across all phases.
  - **Buffers**: `photoCv` (fit-scaled original), `photoMirrorCv` (horizontally flipped copy for mirror-toggle phases), `buf` (the evolving feedback canvas that gets drawn to the main ctx). `resize(W,H)` re-fits all three. Do not read `getImageData` in this path — mobile perf relies on pure `drawImage`.
...
- `VisualAnalyser` (`App.visual`) — Phase 2 driver. Reads a user-uploaded photo (`Image`) or looping video (`<video>`) into a 64×64 offscreen canvas every frame and emits the **same** `bass` / `mid` / `treble` / `volume` shape as `AudioAnalyser` (plus `flowX` / `flowY` for future subject-mode work). Mapping: `volume` = mean luminance, `bass` = large-scale block std-dev + frame-to-frame flow, `mid` = overall contrast, `treble` = edge/gradient energy. Still photos add slow LFO modulation so signal isn't flat. When `visual.active`, the frame loop copies its four signal fields into `this.mic` so every downstream visualizer keeps reading `this.mic.xxx` unchanged — only one driver writes per frame.
  - **Photo subject-mode (narrow scope)**: at `init('photo', file)` time, `_preprocessPhoto()` runs once on a 128×128 offscreen canvas to extract `palette` (top ~6 hues from a saturation-weighted 24-bin histogram) and `edgePoints` (top ~700 Sobel edge cells). These drive two effects: `ColorSystem.applyPalette(palette, t)` pulls `hueBase`/`sLo`/`sHi`/`lLo`/`lHi` toward a slowly-cycling palette each frame (called after `colors.update` in the loop); `App._drawVisualGhost(ctx, W, H, audio)` additively scatters those edge points over the scene with a breathing sinusoid alpha, so the photo's silhouette appears and fades in the background without ever showing pixels. Videos skip both passes — driver-mode signals only.
...
- **Visual input: driver mode is the default; photo subject-mode is explicitly in scope.** Photos get a narrow subject-mode treatment (palette bias + edge-point ghost) because the feature was scoped and shipped. Further subject-mode ideas (video dissolution, kaleido echo, flow-field particles, photo-as-attractor) remain out of scope until scoped the same way.
