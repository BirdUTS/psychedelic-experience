...
2. **Visual input — driver mode only** — add a `VisualAnalyser` that emits the same shape as `AudioAnalyser` (`bass`/`mid`/`treble`/`volume` + optical-flow field) from an uploaded photo or video. Visualizers consume it unchanged. Subject-mode effects (photo dissolution, kaleido echo, flow-field particles) are explicitly out of scope for this phase.
